...
linea = {{'-sb','MarkerFaceColor','blue','MarkerSize',3},{'--or','MarkerFaceColor','red','MarkerSize',3}};

figure

X_sw_upper = zeros(length(Mach),1);
X_sw_lower = zeros(length(Mach),1);
...
for i=1:length(Mach)
    ...
    X_inf_index = find(X_upper >= 0.1,1);
    X_sup_index = find(X_upper >= 0.99,1);
    ...
    X_inf_index = find(X_lower >= 0.1,1);
    X_sup_index = find(X_lower >= 0.99,1);
...
hold on
p = [plot(Mach,X_sw_upper,linea{1}{:}),...
     plot(Mach,X_sw_lower,linea{2}{:})];
hold off

legend(p(:),leyenda,'Location','southeast','interpreter','latex')
